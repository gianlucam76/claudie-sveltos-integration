...
IMAGE_NAME ?= claudie-sveltos-controller
ARCH ?= amd64
...
deploy-projectsveltos: $(KUSTOMIZE)
	...
	@echo 'Load claudie-sveltos-controller image into cluster'
	$(MAKE) load-image
	...
	@echo "Waiting for projectsveltos claudie-sveltos-controller to be available..."
	$(KUBECTL) wait --for=condition=Available deployment/claudie-sveltos-controller -n projectsveltos --timeout=$(TIMEOUT)
...
docker-build: ## Build docker image with the manager.
	docker build --load -t $(CONTROLLER_IMG):$(TAG) .
	MANIFEST_IMG=$(CONTROLLER_IMG) MANIFEST_TAG=$(TAG) $(MAKE) set-manifest-image
